...
def load_dataset() -> pd.DataFrame:
    ...
    # Take first 50 tasks (same as Code Llama benchmark)
    return df.head(50)
...
def benchmark_workflow(
    ...
    """Benchmark the Gemma→LLaMA→DeepSeek workflow on the dataset"""
    ...
    for idx, row in dataset.iterrows():
        ...
        # Show result in completed tasks
        ...
        with completed_tasks_container:
            with st.expander(
                ...
                with col1:
                    ...
                    if result['plan']:
                        # Show COMPLETE plan, no truncation
                        st.text(result['plan'])
                    else:
...
def main():
    st.set_page_config(page_title="Gemma→LLaMA→DeepSeek CoT Benchmark", layout="wide")
    
    st.title("CodeGemma → LLaMA → DeepSeek CoT Benchmark")
    ...
    if st.button("Run Benchmark", type="primary"):
        ...
        if results:
            ...
            filename = f"benchmark_gemma_llama_deepseek_{timestamp}.json"
