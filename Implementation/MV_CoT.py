...
def create_voting_prompt(task: str, solutions: Dict[str, str]) -> str:
    """Create voting prompt where models use chain of thought reasoning before selecting the best solution"""
...
Before providing your final answer, evaluate each solution step-by-step within <thought> tags. 
Then, provide the solution tag name (solutionA, solutionB, or solutionC) inside <best> tags.

Your response must follow this format:
<thought>
[Your step-by-step reasoning and comparison of the solutions]
</thought>
<best>solutionX</best>
</instruction>
...
"""
    ...
    for idx, (model_name, solution) in enumerate(solutions.items()):
        ...
        # Clean solution: only take content up to </solution> tag if present
        cleaned_solution = solution
        if '</solution>' in solution:
            cleaned_solution = solution.split('</solution>')[0]
        
        # Also remove <solution> opening tag if present
        if '<solution>' in cleaned_solution:
            cleaned_solution = cleaned_solution.split('<solution>')[-1]
                ...
                for idx, model_name in enumerate(model_names):
                    with test_cols[idx]:
                        if model_name in all_results:
                            ...
                            st.warning(f"⚠️ {model_name}: Not tested")
                    ...
                    if voting_correct is not None:
                        if voting_correct:
                            ...
                            if num_passing > 0:
                                ...
                                st.warning("⚠️ No passing solutions available")
...
def main():
    ...
    st.title("🗳️ Multi-Model Voting Benchmark")
    ...
    if st.button("🚀 Run Voting Benchmark", type="primary"):
        ...
        if results:
            ...
            if 'voting_accuracy' in analysis:
                st.header("🗳️ Voting Performance")
