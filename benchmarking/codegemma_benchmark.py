...
def load_model():
    """Load CodeGemma model with proper configuration"""
    import os
...
def generate_code(tokenizer, model, device, prompt: str, temperature: float = 0.0, max_tokens: int = 512, top_k: int = 50) -> str:
    """Generate code from CodeGemma model"""
    try:
        with torch.no_grad():
...
def benchmark_codegemma(dataset: pd.DataFrame, temperature: float, max_tokens: int, top_k: int, timeout: int) -> List[Dict]:
    """Benchmark CodeGemma on the entire dataset"""
    ...
    for idx, row in dataset.iterrows():
        ...
        # Create CodeGemma-specific prompt using Gemma chat format
        original_prompt = row['prompt']
        
        # CodeGemma prompt with <start_of_turn> tokens
...
def main():
    st.set_page_config(page_title="CodeGemma Benchmark", layout="wide")
    
    st.title("💎 CodeGemma 7B Instruct Benchmark")
    st.markdown("Benchmarking CodeGemma on McEval Dataset")
    ...
    # CodeGemma-specific notes
    st.sidebar.markdown("---")
    st.sidebar.info("**CodeGemma Settings:**\n- Gemma chat format with <start_of_turn> tokens\n- Uses <solution> tags for extraction")
    
    # Run benchmark
    if st.button("🚀 Run Benchmark", type="primary"):
        st.header(f"Benchmarking: {MODEL_NAME}")
        
        results = benchmark_codegemma(dataset, temperature, max_tokens, top_k, timeout)
        ...
        if results:
            ...
            filename = f"benchmark_codegemma_{timestamp}.json"
