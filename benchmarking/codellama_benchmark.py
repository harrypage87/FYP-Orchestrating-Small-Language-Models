...
# Code Llama Model Configuration
...
def run_unit_test(code: str, test_code: str, entry_point: str, timeout: int = 40) -> Dict:
    ...
    try:
        ...
        if solution_match:
            ...
            # No solution tags found - this shouldn't happen but handle it
...
def load_model():
    """Load Code Llama model with proper configuration"""
    import os
...
def generate_code(tokenizer, model, device, prompt: str, temperature: float = 0.0, max_tokens: int = 512, top_k: int = 50) -> str:
    """Generate code from Code Llama model"""
    try:
        with torch.no_grad():
...
def benchmark_codellama(dataset: pd.DataFrame, temperature: float, max_tokens: int, top_k: int, timeout: int) -> List[Dict]:
    """Benchmark Code Llama on the entire dataset"""
    ...
    for idx, row in dataset.iterrows():
        ...
        # Create Code Llama-specific prompt using [INST] format
        original_prompt = row['prompt']
        
        # Code Llama prompt with [INST] tokens
        prompt = f"""[INST] {original_prompt}
...
def main():
    st.set_page_config(page_title="Code Llama Benchmark", layout="wide")
    
    st.title("🦙 Code Llama 7B Instruct Benchmark")
    st.markdown("Benchmarking Code Llama on McEval Dataset")
    ...
    # Code Llama-specific notes
    st.sidebar.markdown("---")
    st.sidebar.info("**Code Llama Settings:**\n- Llama2 instruction format with [INST] tokens\n- Uses <solution> tags for extraction")
    
    # Run benchmark
    if st.button("🚀 Run Benchmark", type="primary"):
        st.header(f"Benchmarking: {MODEL_NAME}")
        
        results = benchmark_codellama(dataset, temperature, max_tokens, top_k, timeout)
        ...
        if results:
            ...
            filename = f"benchmark_codellama_{timestamp}.json"
